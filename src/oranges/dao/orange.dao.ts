...
import { InjectKnex } from '../../database-modules/knex/knex.decorators';
import { Orange } from '../entities/orange.entity';
import { CreateOrangeDto } from '../dto/create-orange.dto';

@Injectable()
export class OrangeDao {
  private readonly COLUMNS: string[] = ['id', 'name', 'price', 'number'];
  ...
  public async find(): Promise<Orange[]> {
    return this.knex('orange').columns(this.COLUMNS).select();
  }

  public async findOne(id: number): Promise<Orange> {
    return this.knex('orange').columns(this.COLUMNS).where('id', id).first();
  }

  public async delete(id: number): Promise<Orange> {
    return this.knex('orange').columns(this.COLUMNS).where('id', id).delete();
  }
}
